...
def double_load(lr, hr, mode='train', crop_size=32, scale=4, num_frame=7):
    ...
    if mode == 'train':
        ...
        idx=np.random.randint(0,max_frame-1)
    elif mode == 'eval':
    ...
    lr_img=np.array(cv2_imread(lr[idx]))
    hr_img=np.array(cv2_imread(hr[idx]))
    if mode == 'train':
...
class loader(data.Dataset):
    def __init__(self, path, data_kind='single', mode='train', scale=4, crop_size=32, num_frame=7,model='ovsr'):
    ...
    def __getitem__(self, index):
        ...
        if self.data_kind == 'double':
            ...
            LR = torch.from_numpy(LR / 255.).float().permute( 2, 0, 1)#thwc->cthw
        else:
            HR = single_load(self.hqfiles[index], mode=self.mode, crop_size=self.crop_size, scale=self.scale, num_frame=self.num_frame)
