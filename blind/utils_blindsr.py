...
def bicubic_degradation(x, sf=4):
    '''
    Args:
        x: HxWxC image, [0, 1]
...
def degradation_bsrgan(img,shuffle_order, sf=4, lq_patchsize=72, isp_model=None):
    """
...
if __name__ == '__main__':
    ...
    sf = 4
    datafile='../data/filelist_val.txt'
    paths = open(datafile, 'rt').read().splitlines()
    hqname="truth"
    hqfiles=[sorted(glob.glob(join(p, hqname,'*'))) for p in paths]#其中每一个hqfile就是一个video
    i=0
    for hqfile in hqfiles:
        #每个video的降质过程应该是一样的
        shuffle_order = random.sample(range(7), 7)
        idx1, idx2 = shuffle_order.index(2), shuffle_order.index(3)
        if idx1 > idx2:  # keep downsample3 last
            shuffle_order[idx1], shuffle_order[idx2] = shuffle_order[idx2], shuffle_order[idx1]
        videoroot=join(paths[i],"randomlq")#一直到hp001这种
        automkdir(videoroot)
        for frame in hqfile:
            img = util.imread_uint(frame, 3)
            img = util.uint2single(img)
            img_lq, img_hq = degradation_bsrgan(img,shuffle_order=shuffle_order, sf=sf, lq_patchsize=64)
            ...
            lq_path=join(videoroot,frame.split('/')[-1])
            util.imsave(img_lq, lq_path)
        i=i+1
