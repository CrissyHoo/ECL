...
class MtlLearner(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.mode=="meta":
          self.update_lr = config.init_lr
          self.update_step = config.update_step#多少次衰减一下，可以改成用那个函数
          self.base_learner = BaseLearner(config)
        ...
        if self.mode == 'meta':
            ...
            self.decoder= nn.PixelShuffle(4)#upsample的方法,参数是缩放的倍数
    ...
    def pretrain_forward(self, inp):
        ...
        """
        print("after encoder",self.encoder(inp).shape)
